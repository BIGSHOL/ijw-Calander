...
import { ClipboardList, FileText, Users, Bell, Settings, X, Mic, ChevronDown, ChevronUp } from 'lucide-react';
...
const ConsultationDetailModal: React.FC<ConsultationDetailModalProps> = ({
    ...
    const { report: linkedReport } = useConsultationReportStatus(
      consultation.consultationReportId || null
    );
        ...
        <div className="fixed inset-0 bg-black/50 flex items-start justify-center pt-[8vh] z-[100] p-4">
            <div className="bg-white rounded-sm shadow-xl max-w-3xl w-full max-h-[85vh] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-2">
                    ...
                    {/* Section 2.5: 녹음 분석 연동 */}
                    {linkedReport && linkedReport.status === 'completed' && linkedReport.report && (
                        <div className="bg-white border border-gray-200 overflow-hidden">
                            <button
                                onClick={() => setShowReport(!showReport)}
                                className="w-full flex items-center gap-1 px-2 py-1.5 bg-green-50 border-b border-gray-200 hover:bg-green-100 transition-colors"
                            >
                                <Mic className="w-3 h-3 text-green-600" />
                                <h3 className="text-green-800 font-bold text-xs">AI 녹음 분석 보고서</h3>
                                <span className="text-xxs text-green-600 ml-1">({linkedReport.consultationDate})</span>
                                <span className="ml-auto">
                                    {showReport ? <ChevronUp className="w-3 h-3 text-green-600" /> : <ChevronDown className="w-3 h-3 text-green-600" />}
                                </span>
                            </button>
                            {showReport && (
                                <div className="p-2 space-y-2">
                                    {[
                                        { label: '📋 상담 요약', value: linkedReport.report.summary },
                                        { label: '😟 학부모 걱정', value: linkedReport.report.parentConcerns },
                                        { label: '🙋 학부모 요청', value: linkedReport.report.parentRequests },
                                        { label: '📝 학생 특이사항', value: linkedReport.report.studentNotes },
                                        { label: '🤝 합의사항', value: linkedReport.report.agreements },
                                        { label: '✅ 후속 조치', value: linkedReport.report.actionItems },
                                        { label: '🚨 주의 신호', value: linkedReport.report.riskFlags },
                                    ].filter(s => s.value && String(s.value).trim()).map(s => (
                                        <div key={s.label}>
                                            <p className="text-xxs font-semibold text-primary-700">{s.label}</p>
                                            <p className="text-xs text-gray-700 whitespace-pre-wrap leading-relaxed mt-0.5">
                                                <HighlightedReportText content={s.value} />
                                            </p>
                                        </div>
                                    ))}
                                </div>
                    ...
                    {/* 자동 생성 표시 */}
                    {consultation.autoGenerated && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-purple-50 border border-purple-200 rounded-sm">
                            <Mic className="w-3 h-3 text-purple-500" />
                            <span className="text-xxs text-purple-600">녹음 분석에서 자동 생성된 상담 기록입니다</span>
                        </div>
                    )}
