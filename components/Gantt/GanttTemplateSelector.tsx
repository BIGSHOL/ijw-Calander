        ...
        <div className="w-full animate-fade-in-up">
            <div className="flex justify-between items-end mb-6">
                ...
                {canCreate && (
                    <button
                        onClick={onCreateNew}
                        className="flex items-center gap-2 px-4 py-2 bg-white border border-[#081429] text-[#081429] font-medium rounded-lg hover:bg-[#081429] hover:text-[#fdb813] transition shadow-sm"
                    >
            ...
            {templates.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-xl border border-dashed border-slate-300">
                    ...
                    {canCreate && (
                        <button
                            onClick={onCreateNew}
                            className="px-6 py-3 bg-[#081429] text-gray-800 font-medium rounded-lg hover:bg-[#081429]/90 shadow-md transition"
                        >
                            새 프로젝트 만들기
                        </button>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.map((template) => (
                        <div
                            key={template.id}
                            className="group bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-full relative hover:border-[#fdb813] hover:shadow-lg transition-all duration-300"
                        >
                            <div
                            ...
                            <div className="absolute top-4 right-4 flex gap-1">
                                {canEdit && (
                                    <button
                                        ...
                                        className="p-1.5 text-gray-500 hover:text-[#081429] hover:bg-slate-100 rounded-full transition-colors z-10"
                                        title="수정"
                                    >
                                ...
                                {canDelete && (
                                    <button
                                        ...
                                        className="p-1.5 text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors z-10"
                                        title="삭제"
                                    >
