    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full max-h-[500px]">
            ...
            `}
              ...
              ${task.completed ? 'bg-[#fdb813] border-[#fdb813]' : 'bg-white border-slate-300 group-hover:border-[#081429]'}
            `}>
                            {task.completed && (
                                <svg className="w-3.5 h-3.5 text-[#081429]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-center mb-1">
                                <span className={`text-sm font-medium truncate ${task.completed ? 'text-[#373d41]/60 line-through' : 'text-[#081429]'}`}>
                                    {index + 1}. {task.title}
                                </span>
                                <span className="text-xs text-[#373d41]/70 bg-slate-100 px-1.5 py-0.5 rounded">
                                    {task.duration}일
                                </span>
                            </div>
                            <p className={`text-xs ${task.completed ? 'text-[#373d41]/50' : 'text-[#373d41]'}`}>
                                {task.description}
                            </p>
                        </div>
                    </div>
                ...
                {tasks.length === 0 && (
                    <div className="text-center p-8 text-[#373d41]/50 text-sm">
                        임무를 기다리는 중...
                    </div>
                )}
            </div>
