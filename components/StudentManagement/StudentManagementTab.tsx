...
const StudentManagementTab: React.FC<StudentManagementTabProps> = ({ filters, sortBy, currentUser, initialSelectedStudentId, onStudentSelected }) => {
  ...
  // master는 전체 학생 보기 (자동전체보기), 그 외는 소속 기반 필터링
  const effectiveDepartments = currentUser?.role === 'master' ? undefined : currentUser?.departments;
  const filteredStudents = useStudentFilters(students, filters, sortBy, oldWithdrawnStudents, effectiveDepartments);
