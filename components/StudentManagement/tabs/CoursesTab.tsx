...
const CoursesTab: React.FC<CoursesTabProps> = ({ student, compact = false, readOnly = false, currentUser }) => {
  ...
  // 삭제된 퇴원 기록 (hiddenAt 있는 enrollment) — admin 전용
  // 시간표 수정 모드에서 퇴원 학생을 Delete 키로 삭제 시 hiddenAt 마크됨.
  // 복원: ⤴︎ 클릭 → hiddenAt 필드 제거 → 시간표 퇴원 섹션에 다시 표시.
  ...
  const hiddenEnrollments = useMemo<HiddenEntry[]>(() => {
    const list: HiddenEntry[] = [];
    (student.enrollments || []).forEach((e: any) => {
      if (!e.hiddenAt) return;
      list.push({
        id: e.id,
        className: e.className,
        subject: e.subject,
  ...
  // 숨겨진 퇴원 record 복원 — hiddenAt/hiddenBy 필드 제거
  const handleRestoreHidden = async (enrollmentId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!canManageClassHistory) {
      alert('숨겨진 퇴원 기록을 복원할 권한이 없습니다.');
      return;
    }
    if (!window.confirm('이 퇴원 기록을 복원하시겠습니까? 시간표의 퇴원 섹션에 다시 표시됩니다.')) return;

    setRestoringEnrollmentId(enrollmentId);
    try {
      const ref = doc(db, 'students', student.id, 'enrollments', enrollmentId);
      await updateDoc(ref, {
      ...
      console.error('퇴원 기록 복원 오류:', err);
      alert('퇴원 기록 복원에 실패했습니다.');
    } finally {
      setRestoringEnrollmentId(null);
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {/* 삭제된 퇴원 기록 섹션 — admin/master 전용. 시간표 수정모드 Delete 키로 숨긴 record 복원 */}
      {canManageClassHistory && hiddenEnrollments.length > 0 && (
        <div className="mt-4">
          <div
            className="flex items-center gap-2 mb-2 cursor-pointer"
            onClick={() => setShowCancelledClasses(!showCancelledClasses)}
          >
            <h3 className="text-xs font-bold text-gray-700">삭제된 퇴원 기록</h3>
            <span className="text-xs text-gray-700">
              ({hiddenEnrollments.length}개)
            </span>
            <span className="text-xxs text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded-sm border border-gray-200">
              관리자 전용 · 복원 가능
            </span>
            <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${showCancelledClasses ? '' : 'rotate-180'}`} />
          </div>
          {showCancelledClasses && (
            <div className="bg-white border border-gray-200 overflow-hidden">
              <div className="flex items-center gap-2 px-2 py-1 bg-gray-50/60 border-b border-gray-200 text-xxs font-medium text-gray-700">
                <span className="w-8 shrink-0">과목</span>
                <span className="flex-1 min-w-0">수업명</span>
                <span className="w-14 shrink-0">강사</span>
                <span className="w-20 shrink-0 text-center">시작</span>
                <span className="w-20 shrink-0 text-center">종료</span>
                <span className="w-20 shrink-0 text-center">삭제일</span>
                <span className="w-5 shrink-0"></span>
              </div>
              {hiddenEnrollments.map((h) => {
                const subjectColor = SUBJECT_COLORS[h.subject as keyof typeof SUBJECT_COLORS] || SUBJECT_COLORS.other;
                const teacherName = getTeacherByIdOrName(h.staffId)?.name || h.staffId || '-';
                const isRestoring = restoringEnrollmentId === h.id;
                return (
                  <div
                    key={`hidden-${h.id}`}
                    className="flex items-center gap-2 px-2 py-1.5 border-b border-gray-100 opacity-70"
                  >
                    <span
                      className="w-8 shrink-0 text-micro px-1 py-0.5 rounded-sm font-semibold text-center"
                      style={{ backgroundColor: subjectColor.bg, color: subjectColor.text }}
                    >
                      {SUBJECT_LABELS[h.subject as keyof typeof SUBJECT_LABELS]}
                    </span>
                    <span className="flex-1 min-w-0 text-xs text-primary-700 truncate">
                      {h.className}
                    </span>
                    ...
                    <span className="w-20 shrink-0 text-xxs text-primary-700 text-center">
                      {h.startDate ? formatDate(h.startDate) : '-'}
                    </span>
                    <span className="w-20 shrink-0 text-xxs text-primary-700 text-center">
                      {h.endDate ? formatDate(h.endDate) : '-'}
                    </span>
                    <span className="w-20 shrink-0 text-xxs text-gray-600 text-center font-bold">
                      {formatDate(h.hiddenAt)}
                    </span>
                    {!readOnly && (
                      <button
                        onClick={(e) => handleRestoreHidden(h.id, e)}
                        disabled={isRestoring}
                        className="w-5 h-5 shrink-0 flex items-center justify-center text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-sm transition-colors disabled:opacity-50"
                        title="이 퇴원 기록을 시간표에 다시 표시"
                      >
                        {isRestoring ? (
                          <Loader2 className="w-3 h-3 animate-spin" />
                        ) : (
