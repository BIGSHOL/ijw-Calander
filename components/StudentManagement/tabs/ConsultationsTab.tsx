...
const ConsultationsTab: React.FC<ConsultationsTabProps> = ({ student }) => {
  ...
  // 등록 상담 이력 조회 (registeredStudentId로 연결)
  const { data: registrationHistory = [], isLoading: regHistoryLoading } = useConsultations({ studentId: student.id });

  // Performance: rerender-memo - 최신순 정렬 메모이제이션
          ...
          <div className="space-y-2">
            {registrationHistory.map(record => (
              <div key={record.id} className="bg-white rounded-md p-2 border border-blue-100">
                ...
                <div className="text-xs text-gray-700 line-clamp-2">{record.notes || '내용 없음'}</div>
                {record.consultationPath && (
                  <div className="mt-1 text-xxs text-gray-500">경로: {record.consultationPath}</div>
                )}
              </div>
            ))}
          </div>
