...
const SortableItem = ({ level, onMoveUp, onMoveDown, onEdit, onDelete, isFirst, isLast }: SortableItemProps) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className="flex items-center justify-between p-2 bg-white border rounded mb-2 shadow-sm group hover:border-indigo-300 transition-colors">
            ...
            <div className="flex items-center gap-1 ml-2">
                ...
                <button
                    onClick={onMoveUp}
                    disabled={isFirst}
                    className={`p-1 rounded ${isFirst ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-indigo-600 hover:bg-gray-100'}`}
                >
                    <ChevronUp size={16} />
                </button>
                <button
                    onClick={onMoveDown}
                    disabled={isLast}
                    className={`p-1 rounded ${isLast ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-indigo-600 hover:bg-gray-100'}`}
                >
                    <ChevronDown size={16} />
                </button>
                <div className="w-px h-4 bg-gray-200 mx-1"></div>
                ...
                <div className="flex-1 overflow-y-auto p-5 bg-gray-100/50">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-4">
                        ...
                        <div className="flex gap-2 mb-2">
                            <div className="w-1/3">
                                <input
                                    ...
                                    placeholder="약어 (예: DP)"
                                    className="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                                />
                            </div>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1 px-4 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded hover:bg-indigo-700 transition-colors"
                            >
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center mb-1 px-1">
                            <span className="text-xs font-bold text-gray-500">레벨 목록 ({levels.length})</span>
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-8 bg-white rounded-lg border border-dashed border-gray-300 text-gray-400 text-sm">
                                등록된 레벨이 없습니다.<br />
                                위 '기본값 불러오기'를 눌러 초기화하세요.
                            </div>
                        ) : (
                            <DndContext
                                sensors={sensors}
