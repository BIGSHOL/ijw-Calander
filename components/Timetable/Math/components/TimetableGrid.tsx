                ...
                <div className="relative">
                    <table ref={tableRef ?? undefined} className="border-collapse border-[3px] border-black" style={{ tableLayout: 'fixed', width: `${totalTableWidth}px` }}>
                        ...
                        <thead className="sticky top-0 z-20">
                            ...
                            <tr>
                                {!hidePeriodColumn && <th
                                    className={`p-1.5 text-period-label font-bold text-black border-b border-b-black border-r-[6px] border-r-black sticky left-0 z-30`}
                                    rowSpan={2}
                                    style={{
                                        width: '90px',
                                        minWidth: '90px',
                                ...
                                {resources.map(resource => {
                                    ...
                                    return (
                                        <th
                                            key={resource}
                                            colSpan={colspan}
                                            className="p-1.5 text-sm font-bold border-b-[6px] border-b-black border-r-[6px] border-r-black truncate"
                                            style={{
                                                ...headerWidthStyle,
                            ...
                            <tr>
                                {resources.map(resource => {
                                    ...
                                    return daysForResource.map((day, dayIndex) => {
                                        ...
                                        const borderRightClass = 'border-r-[6px] border-r-black';
                                        ...
                                        return (
                                            <th
                                                key={`${resource}-${day}`}
                                                className={`p-1.5 text-xxs font-bold text-center border-b-[6px] border-b-black ${borderRightClass} ${isWeekend ? 'bg-orange-50 text-black' : day === '수' ? 'bg-green-50 text-black' : 'bg-gray-100 text-black'}`}
                                        ...
                                        if (hasFirst && hasSecond) {
                                            ...
                                            rows.push(
                                                <tr key={`group-${groupId}`}>
                                                    {!hidePeriodColumn && (
                                                        <td
                                                            className={`p-1.5 text-period-label font-bold text-black text-center sticky left-0 z-10 border-b-[6px] border-b-black border-r-2 border-r-black`}
                                                            style={{
                                                                width: '90px',
                                                                minWidth: '90px',
                                                        ...
                                                        while (dayIndex < daysForResource.length) {
                                                            ...
                                                            cells.push(
                                                                <td
                                                                    ...
                                                                    className={`p-0 align-top border-b-[6px] border-b-black ${borderRightClass}`}
                                    ...
                                    return currentPeriods.map(period => {
                                        ...
                                        return (
                                            <tr key={period}>
                                                {!hidePeriodColumn && (
                                                    <td
                                                        className={`p-1.5 text-period-label font-bold text-black text-center sticky left-0 z-10 border-b-[6px] border-b-black border-r-[6px] border-r-black`}
                                                        style={{
                                                            width: '90px',
                                                            minWidth: '90px',
                                                            backgroundColor: bgHex
                                                ...
                                                {resources.map(resource => {
                                                    ...
                                                    while (dayIndex < daysForResource.length) {
                                                        ...
                                                        const isEmpty = cellClasses.length === 0;
                                                        const isLastDayForResource = (dayIndex + colSpan - 1) === daysForResource.length - 1;
                                                        const borderRightClass = isLastDayForResource ? 'border-r-[6px] border-r-black' : 'border-r border-r-black';
                        ...
                        <thead className="sticky top-0 z-20">
                            ...
                            <tr>
                                ...
                                {resources.map((resource, idx) => {
                                    ...
                                    const borderRightClass = isLast ? 'border-r-[6px] border-r-black' : 'border-r border-r-black';

                                    return (
                                        <th
                                            key={resource}
                                            className={`p-1.5 text-sm font-bold border-b-[6px] border-b-black truncate ${borderRightClass}`}
                                            style={{
                                                ...singleCellWidthStyle,
                                                backgroundColor: bgColor,
                                                color: textColor
                                                ...
                                                rows.push(
                                                    <tr key={`period-${period}`}>
                                                        ...
                                                        {resources.map((resource, resourceIdx) => {
                                                            ...
                                                            const isEmpty = cellClasses.length === 0;
                                                            const isLastResource = resourceIdx === resources.length - 1;
                                                            const borderRightClass = isLastResource ? 'border-r-[6px] border-r-black' : 'border-r border-r-black';
            ...
            <div className="flex-shrink-0 sticky left-0 z-30">
                <table ref={periodColRef} className="border-collapse border-[3px] border-black" style={{ tableLayout: 'fixed', width: '90px' }}>
                    ...
                    <thead className="sticky top-0 z-20">
                        ...
                        <tr>
                            <th className="p-1.5 text-period-label font-bold text-black border-b-[6px] border-b-black" rowSpan={2} style={{ width: '90px', minWidth: '90px', backgroundColor: bgHex }}>교시</th>
                        </tr>
                        <tr />
                    </thead>
                    <tbody className="timetable-body">
                        {rows.map((row, i) => (
                            <tr key={`period-col-${i}`}>
                                <td
                                    className="p-1.5 text-period-label font-bold text-black text-center border-b-[6px] border-b-black"
                                    style={{
        ...
        const buildMergedPeriodCells = (
            ...
            return config.resources.flatMap(resource => {
                ...
                while (dayIndex < daysForResource.length) {
                    ...
                    const borderR = isGroupBoundary ? 'border-r-[6px] border-r-black' : (isLastDay ? 'border-r-2 border-r-black' : (bothEmpty ? '' : 'border-r border-r-black'));

                    cells.push(
                        <td key={`${groupName}-${resource}-${day}-${groupId}`}
                            className={`p-0 align-top border-b-[6px] border-b-black ${borderR}`}
                            style={{ ...cellStyle, ...(bothEmpty ? { backgroundColor: EMPTY_CELL_BG } : {}) }}
                            colSpan={colSpan > 1 ? colSpan : undefined}
        ...
        const buildSinglePeriodCells = (
            ...
            return config.resources.flatMap(resource => {
                ...
                while (dayIndex < daysForResource.length) {
                    ...
                    const isLastDay = (dayIndex + colSpan - 1) === daysForResource.length - 1;
                    const isGroupBoundary = isLastDay && resource === lastResource;
                    const borderR = isGroupBoundary ? 'border-r-[6px] border-r-black' : (isLastDay ? 'border-r-2 border-r-black' : 'border-r border-r-black');
                    ...
                    if (hasWeekendCols) {
                        ...
                        rows.push(
                            <tr key={`unified-group-${groupId}-1`}>
                                <td className="p-1.5 text-period-label font-bold text-black text-center sticky left-0 z-10 border-b-[6px] border-b-black border-r-2 border-r-black"
                        ...
                        rows.push(
                            <tr key={`unified-group-${groupId}-2`}>
                                ...
                                <td className="p-1 text-xxs font-bold text-black text-center border-b-[6px] border-b-black border-r border-r-black"
                                    style={{ width: '60px', minWidth: '60px', minHeight: '60px', backgroundColor: '#fff7ed' }}>
                            ...
                            {groupColumnInfo.flatMap(g => g.cols.map((c, i) => {
                                ...
                                return (
                                    <th key={`${g.groupName}-${c.resource}-${c.day}`}
                                        className={`p-1.5 text-xxs font-bold text-center bg-gray-100 text-black border-t-2 border-b-[6px] border-r-2 border-black`}
                                        style={dayW}>
                                        <div>{c.day}</div>
                                        {dateInfo && <div className="text-xxs opacity-70">{dateInfo.formatted}</div>}
                                    </th>
                            ...
                            {hasWeekendCols && weekendColInfo!.cols.map((c, i) => {
                                ...
                                return (
                                    <th key={`weekend-${c.resource}-${c.day}`}
                                        className={`p-1.5 text-xxs font-bold text-center bg-orange-50 text-black border-t-2 border-b-[6px] border-r-2 border-black`}
                                        style={dayW}>
                                        <div>{c.day}</div>
                                        {dateInfo && <div className="text-xxs opacity-70">{dateInfo.formatted}</div>}
                                    </th>
