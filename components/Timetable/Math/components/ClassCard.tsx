            ...
            {showStudents && (
                isMergedCell ? (
                    <div className="flex-1 flex flex-col min-w-0 min-h-0">
                        <div
                            className={`flex-1 px-1 py-0 transition-colors min-w-0 min-h-0 overscroll-auto ${dragOverZone === 'common' ? 'bg-indigo-100 ring-2 ring-inset ring-indigo-400' : ''}`}
                            onDragOver={(e) => {
                                if (!canEdit) return;
                                e.preventDefault();
                                e.stopPropagation();
                        ...
                        >
                            <div className={`${fontSizeClass} text-indigo-600 mb-0 overflow-hidden whitespace-nowrap`}><span className="font-bold" style={{ fontSize: 'calc(1em + 1px)' }}>{commonStudents.active.length + partialActiveCount}명</span> - 재원생</div>
                    ...
                    <div className="flex-1 flex flex-col min-w-0 min-h-0">
                        <div className="flex-1 px-0.5 py-0.5 min-w-0 min-h-0 overscroll-auto">
                            <div className={`${fontSizeClass} text-indigo-600 mb-0 overflow-hidden whitespace-nowrap`}><span className="font-bold" style={{ fontSize: 'calc(1em + 1px)' }}>{activeStudents.length}명</span> - 재원생</div>
