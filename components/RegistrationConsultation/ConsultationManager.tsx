...
const ConsultationManager: React.FC<ConsultationManagerProps> = ({ userProfile, onNavigateToStudent }) => {
    ...
    const openDraftAsForm = (draft: ConsultationDraft) => {
        // 되돌리기로 생성된 draft 면 본문 snapshot 우선 복원 → 학생/학부모 기본은 draft 최신값으로 보정
        const snap = draft.consultationSnapshot || {};
        const prefilled: ConsultationRecord = {
            ...(snap as Partial<ConsultationRecord>),
            id: '',
            studentName: draft.studentName,
            gender: draft.gender,
            bloodType: draft.bloodType || snap.bloodType || '',
            schoolName: draft.schoolName,
            grade: (draft.grade as SchoolGrade) || (snap.grade as SchoolGrade) || SchoolGrade.Middle1,
            studentPhone: draft.studentPhone || snap.studentPhone || '',
            parentName: draft.parentName,
            parentRelation: draft.parentRelation || snap.parentRelation || '모',
            parentPhone: draft.parentPhone,
            address: draft.address || snap.address || '',
            careerGoal: draft.careerGoal || snap.careerGoal || '',
            siblings: draft.siblings || snap.siblings || '',
            ...
            consultationPath: draft.consultationPath || snap.consultationPath || '',
            // 직원이 채울 필드 — snapshot 있으면 그대로, 아니면 기본값
            birthDate: snap.birthDate || '',
            safetyNotes: snap.safetyNotes || '',
            siblingsDetails: snap.siblingsDetails || '',
            studentType: snap.studentType || '',
            consultationDate: snap.consultationDate || new Date().toISOString().slice(0, 10),
            subject: (draft.subjects?.[0] as ConsultationSubject) || snap.subject || ConsultationSubject.Math,
            status: snap.status || ConsultationStatus.BeforeConsultation,
            counselor: snap.counselor || '',
            receiver: snap.receiver || '',
            registrar: snap.registrar || '',
            paymentAmount: snap.paymentAmount || '',
            paymentDate: snap.paymentDate || '',
            notes: snap.notes || '',
            nonRegistrationReason: snap.nonRegistrationReason || '',
            followUpDate: snap.followUpDate || '',
            followUpContent: snap.followUpContent || '',
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 font-sans text-slate-900">
            {/* Desktop Header - Light Theme (수학 시간표 스타일) */}
            <TabSubNavigation
                variant="compact"
                theme="light"
                showBorder={true}
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    className="px-2 py-1 rounded-sm bg-white border border-gray-300 text-gray-700 text-xs font-bold focus:border-accent focus:ring-1 focus:ring-accent outline-none"
                                >
                                ...
                                <div className="flex items-center bg-white rounded-sm border border-gray-300">
                                    <button
                                    ...
                                    <select
                                        ...
                                        disabled={selectedYear === 'all'}
                                        className="bg-transparent text-gray-700 text-xs font-bold px-1 py-0.5 outline-none text-center min-w-[50px] cursor-pointer disabled:opacity-50"
                                    >
                    ...
                    {view === 'table' && (
                        <>
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    className="w-40 pl-7 pr-2 py-1 rounded-sm text-xs bg-white border border-gray-300 text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent"
                                    placeholder="학생명, 번호 검색..."
                                    value={searchTerm}
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around items-center h-20 pb-4 z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                ...
                >
                    ...
                    <span className="text-xxs font-medium">통계</span>
                </button>
                <button
                    onClick={() => setView('table')}
                ...
                >
                    ...
                    <span className="text-xxs font-medium">목록</span>
                </button>
            </div>
