...
### 🎨 design-system-guardian (디자인 시스템 수호자)

#### 핵심 역할
**UI/UX 일관성 점검 및 디자인 시스템 관리**
...
#### 하지 않는 것 ❌
- 코드 로직 검토 (→ code-reviewer)
- 실제 수정 (→ refactor-expert)
...
#### 하지 않는 것 ❌
- Firebase 비용 분석 (→ firebase-cost-optimizer)
- 코드 리팩토링 (→ refactor-expert)

---

### 🔄 migration-helper (마이그레이션 도우미)

#### 핵심 역할
**데이터/스키마 마이그레이션 및 하위 호환성 관리**
...
### 10. 성능 최적화 플로우
```
performance-optimizer → firebase-cost-optimizer → refactor-expert
```

### 11. 데이터 마이그레이션 플로우
```
academy-domain-expert → migration-helper → test-writer
```
