...
6. **상담 녹음 분석 연동** - 상담 녹음 분석과 상담 기록 간의 양방향 연동이 올바른지 검증
...
- `components/ConsultationRecording/` 수정 후
- `hooks/useStudents.ts`, `hooks/useConsultations.ts`, `hooks/useGradeProfile.ts` 수정 후
- `hooks/useConsultationRecording.ts` 또는 `utils/formatReportContent.ts` 수정 후
...
| `components/StudentConsultation/ConsultationDetailModal.tsx` | 상담 상세 모달 (녹음 분석 연동) |
| `components/ConsultationRecording/ConsultationRecordingTab.tsx` | 상담 녹음 분석 메인 탭 (2단 레이아웃) |
| `components/ConsultationRecording/ReportViewer.tsx` | 녹음 분석 보고서 뷰어 |
| `components/ConsultationRecording/RecordingUploader.tsx` | 녹음 업로드 폼 (학생 자동완성) |
| `components/ConsultationRecording/ReportHistoryList.tsx` | 분석 내역 목록 |
| `hooks/useConsultationRecording.ts` | 녹음 분석 Firestore 실시간 구독 + mutation |
| `types/consultation.ts` | 상담 타입 (consultationReportId, autoGenerated 포함) |
| `types/consultationReport.ts` | 녹음 분석 보고서 타입 |
| `utils/formatReportContent.ts` | 보고서 줄바꿈 정규화 유틸 |
| `utils/HighlightedReportText.tsx` | ASR [?] 하이라이트 렌더링 컴포넌트 |
...
### Step 8: 상담 녹음 분석 연동 검증

**검사:** 상담 상세 모달에서 녹음 분석 보고서를 연동하는 구조가 올바른지 확인합니다.

```bash
# ConsultationDetailModal에서 녹음 분석 연동 확인
grep -n "consultationReportId\|useConsultationReportStatus\|HighlightedReportText" components/StudentConsultation/ConsultationDetailModal.tsx
```

**PASS 기준:** `consultationReportId`, `useConsultationReportStatus`, `HighlightedReportText` 모두 참조됨
**FAIL 기준:** 연동 코드 누락

### Step 9: 상담 녹음 분석 탭 2단 레이아웃 검증

**검사:** ConsultationRecordingTab에서 분석 완료 시 2단 그리드 레이아웃이 적용되는지 확인합니다.

```bash
grep -n "grid-cols\|RecordingUploader\|ReportViewer" components/ConsultationRecording/ConsultationRecordingTab.tsx
```

**PASS 기준:** `grid-cols` 패턴 존재 + RecordingUploader와 ReportViewer가 분리된 div에 위치
**FAIL 기준:** 1단 레이아웃만 사용

### Step 10: HighlightedReportText 일관 사용 검증

**검사:** 보고서 텍스트를 렌더링하는 모든 위치에서 HighlightedReportText를 사용하는지 확인합니다.

```bash
# formatReportContent를 JSX 내에서 직접 호출하는 곳이 없어야 함 (TXT 다운로드 제외)
grep -rn "formatReportContent" components/ --include="*.tsx" | grep -v "import" | grep -v "// " | grep -v "handleDownload\|lines.push"
```

**PASS 기준:** JSX 렌더링에서 formatReportContent 직접 호출 없음 (HighlightedReportText만 사용)
**FAIL 기준:** JSX에서 formatReportContent를 직접 호출하는 곳이 남아있음

**수정:** `formatReportContent(content)` → `<HighlightedReportText content={content} />` 교체
...
| 8 | 녹음 분석 연동 | ConsultationDetailModal | PASS/FAIL | |
| 9 | 녹음 탭 2단 레이아웃 | ConsultationRecordingTab | PASS/FAIL | |
| 10 | HighlightedReportText 일관 사용 | components/ | PASS/FAIL | |
...
6. **ReportViewer의 TXT 다운로드에서 formatReportContent 직접 사용** - 텍스트 파일 생성에는 JSX 렌더링이 불필요하므로 문자열 함수 직접 호출이 정상
7. **ConsultationRecordingTab의 1단↔2단 조건부 레이아웃** - 분석 완료 전에는 1단, 완료 후에는 2단 그리드로 전환하는 것은 정상 UX 패턴
