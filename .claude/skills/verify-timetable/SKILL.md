...
| `components/Timetable/Math/hooks/useMathClassStudents.ts` | 수학 수업 학생 데이터 훅 (useSubjectClassStudents 래퍼) |
| `components/Timetable/English/hooks/useClassStudents.ts` | 영어 수업 학생 데이터 훅 (useSubjectClassStudents 래퍼 + 시뮬레이션) |
| `components/Timetable/Generic/hooks/useClassStudents.ts` | 범용 수업 학생 데이터 훅 (useSubjectClassStudents 래퍼) |
| `hooks/useSubjectClassStudents.ts` | 과목별 학생 데이터 통합 훅 (수학/영어/Generic 공통 로직) |
| `utils/firestoreConverters.ts` | Firestore Timestamp → 날짜 변환 (convertTimestampToDate 단일 소스) |
| `utils/enrollment.ts` | 수강종료 판정 (getEndedSubjects 단일 소스) |
