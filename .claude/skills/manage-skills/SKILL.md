...
| `verify-firebase-mutations` | Firebase mutation 훅의 invalidateQueries/에러처리 패턴 검증 | `hooks/use*Mutations.ts`, `hooks/use*.(ts|tsx)` (useMutation 사용 38개 훅) |
...
| `verify-student-tabs` | 학생 그룹(학생/상담/성적/퇴원/계약/성적표) + 상담녹음분석 연동 검증 | `components/StudentManagement/**`, `components/*Consultation/**`, `components/ConsultationRecording/**`, `components/Grades/**`, `components/WithdrawalManagement/**`, `components/Contracts/**`, `components/Reports/**`, `utils/enrollment.ts`, `utils/firestoreConverters.ts`, `utils/formatReportContent.ts`, `utils/HighlightedReportText.tsx`, `hooks/useConsultationRecording.ts` |
| `verify-admin-tabs` | 관리 그룹(수강료계산/수강료현황/직원/수납/자료실/역할/통계/급여) 탭 기능 검증 | `components/TuitionCalculator/**`, `components/PaymentReport/**`, `components/Staff/**`, `components/Billing/**`, `components/Resources/**`, `components/RoleManagement/**`, `components/Analytics/**`, `components/Payroll/**` |
...
| `verify-tuition-tabs` | 수강료 계산기 탭 (3모드, DB 카테고리 매핑, 출석부 동기화, 학생 자동완성) 검증 | `components/TuitionCalculator/**`, `hooks/useTuition*.ts`, `types/tuition.ts`, `constants/tuition*.ts`, `utils/tuitionHolidays.ts` |
