...
/**
 * Claude Messages API 를 streaming(SSE) 모드로 호출하여 전체 응답 텍스트를 반환.
 *
 * 사용 이유:
 * - max_tokens 가 큰 호출(64K 등)은 non-streaming 시 Anthropic 의 10분 timeout 정책에 막힘
 *   ("Streaming is required for operations that may take longer than 10 minutes")
 * - 본 helper 는 body 에 stream: true 를 자동 부여하고 SSE 청크를 누적하여 최종 텍스트 반환
 *
 * 반환:
 *   { ok: true,  text, usage, stopReason }
 *   { ok: false, status, statusText, errorBody }
 *
 * 주의: 호출자는 기존 `claudeResponse.json().content[0].text` 패턴 대신 `result.text` 사용.
 */
async function fetchClaudeStreamingText(url, options, maxRetries = 3) {
    // body 에 stream: true 강제 부여 (이미 있어도 덮어씀)
    let parsedBody;
    try {
        parsedBody = JSON.parse(options.body);
    } catch (e) {
        throw new Error('[fetchClaudeStreamingText] options.body 가 JSON 문자열이어야 합니다');
    }
    parsedBody.stream = true;
    const streamOptions = { ...options, body: JSON.stringify(parsedBody) };
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (!response.ok) {
            ...
            const delay = Math.min(1000 * Math.pow(2, attempt) + Math.random() * 1000, 15000);
            logger.warn(`[fetchClaudeStreamingText] ${response.status} 응답, ${Math.round(delay)}ms 후 재시도 (${attempt + 1}/${maxRetries})`);
            await new Promise(r => setTimeout(r, delay));
            continue;
        }
        ...
        try {
            ...
            // 종료 후 buffer 잔여 처리 (보통 빈 문자열)
            return { ok: true, text: fullText, usage, stopReason };
        } catch (streamErr) {
            // streaming 도중 끊김 → 재시도
            const retryable = attempt < maxRetries - 1;
            if (retryable) {
                const delay = Math.min(1000 * Math.pow(2, attempt) + Math.random() * 1000, 15000);
                logger.warn(`[fetchClaudeStreamingText] stream 끊김, ${Math.round(delay)}ms 후 재시도 (${attempt + 1}/${maxRetries}):`, streamErr?.message || streamErr);
                await new Promise(r => setTimeout(r, delay));
                continue;
            }
            return { ok: false, status: 0, statusText: 'stream_error', errorBody: streamErr?.message || String(streamErr) };
        }
    }
    return { ok: false, status: 0, statusText: 'max_retries_exceeded', errorBody: '' };
}
...
async function scrapeMakeEduShuttleStudentsInternal() {
    ...
    logger.info(`[scrapeMakeEduShuttle] KST: ${kstYear}-${kstMonth}-${kstDay}, 당월: ${currentYM}, 전월확인: ${needPrevMonth ? prevYM : '불필요'}`);

    // 1. Puppeteer 브라우저 시작 + 로그인 + 상세수납 페이지 이동
    let browser = null;
    try {
        browser = await puppeteer.launch({
            args: chromium.args,
        ...
        try {
            ...
            logger.info("[scrapeMakeEduShuttle] Login nav:", navErr.message);
            await new Promise(r => setTimeout(r, 5000));
        }
        logger.info("[scrapeMakeEduShuttle] Logged in, URL:", page.url());

        // 2. 상세수납 페이지 이동
        logger.info("[scrapeMakeEduShuttle] Navigating to billing page...");
        await page.goto(billingUrl, { waitUntil: "networkidle2", timeout: 60000 });
        await new Promise(r => setTimeout(r, 5000));

        // listSize 500으로 설정
        let listSizeSet = false;
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                await page.waitForSelector('#listSize', { timeout: 10000 });
                await page.select('#listSize', '500');
                listSizeSet = true;
                logger.info("[scrapeMakeEduShuttle] Set listSize to 500");
                break;
            } catch (e) {
                logger.info(`[scrapeMakeEduShuttle] listSize attempt ${attempt + 1} failed`);
                await new Promise(r => setTimeout(r, 3000));
            }
        }
        if (!listSizeSet) {
        ...
        try {
            await Promise.all([
                ...
                page.evaluate(() => {
                    // 메이크에듀 정확한 셀렉터: <a id="btnSrch" class="basic_btn">검색</a>
                    const btn = document.querySelector('#btnSrch') ||
        ...
        while (pageNum <= maxPages) {
            logger.info(`[scrapeMakeEduShuttle] Scraping billing page ${pageNum}...`);
            ...
            const pageData = await page.evaluate(() => {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    // 학생명
                    let nameLink = row.querySelector('a.dl_pop.st_') || row.querySelector('a.dl_pop') || row.querySelector("a[onclick*='stuInfo']");
                    if (!nameLink) {
                        const tdLinks = row.querySelectorAll('td a');
                        for (let n = 0; n < tdLinks.length; n++) {
                            const t = tdLinks[n].innerText.trim();
                            if (t.length >= 2 && t.length <= 5) { nameLink = tdLinks[n]; break; }
                        }
                    }
                    ...
                    for (let j = 0; j < tds.length; j++) {
                        if (tds[j].querySelector('input[name="payReqDd"]') && j + 1 < tds.length) {
                            category = tds[j + 1].innerText.trim();
                            break;
                        }
                    }

                    // 수업명 (a.btnPayName) — 메이크에듀 사이트마다 셀렉터 다를 수 있어 fallback 추가
                    const payNameLink = row.querySelector('a.btnPayName');
                    let className = payNameLink ? payNameLink.innerText.trim() : '';

                    // Fallback: 모든 td 텍스트를 합쳐서 backup으로 보관
                    const allText = Array.from(tds).map(td => (td.innerText || '').trim()).join(' | ');

                    // className이 비어있으면 allText 사용 (셀렉터 미스매치 보호)
                    if (!className) className = allText;

                    data.push({ studentName, billingMonth, category, className, allText });
                }
                return data;
            });

            logger.info(`[scrapeMakeEduShuttle] Page ${pageNum}: ${pageData.length} rows`);
            allBillingRows.push(...pageData);

            if (pageData.length === 0) break;

            // 다음 페이지 이동
            const hasNext = await page.evaluate((currentPage) => {
                const pagingLinks = document.querySelectorAll('.paging a.gotoPage, .paging a');
                for (let i = 0; i < pagingLinks.length; i++) {
                    const link = pagingLinks[i];
                    const title = link.getAttribute('title') || '';
                    const text = link.innerText.trim();
                    const nextPageStr = String(currentPage + 1);
                    if (title.includes(nextPageStr) || text === nextPageStr) {
                        link.click();
                        return true;
                    }
                }
            ...
            if (!hasNext) {
                logger.info(`[scrapeMakeEduShuttle] No more pages after ${pageNum}`);
                break;
            }
            ...
            await new Promise(r => setTimeout(r, 3000));
            try {
                await page.waitForSelector('tr.list_tr, tr[id^="tr_"]', { timeout: 15000 });
            } catch (e) {}
            await new Promise(r => setTimeout(r, 2000));
        }

        logger.info(`[scrapeMakeEduShuttle] Total billing rows: ${allBillingRows.length}`);

        // 4. 셔틀버스 학생 필터링
        // 청구월 정규화 (2026.04 → 2026-04, 2026-04 → 2026-04)
        const normalizeYM = (ym) => (ym || '').replace(/[./]/g, '-').substring(0, 7);

        const shuttleNameSet = new Set();
        const shuttleDetails = []; // 디버그용

        // 디버그: 첫 5개 row 샘플 출력 (필드 구조 확인용)
        logger.info(`[scrapeMakeEduShuttle] Row sample (first 5):`,
            JSON.stringify(allBillingRows.slice(0, 5)));

        allBillingRows.forEach(row => {
            const ym = normalizeYM(row.billingMonth);
            // className + category + allText 모두 합쳐서 검색 (셀렉터 미스매치 보호)
            const searchText = [
                row.className || '',
                row.category || '',
                row.allText || '',
            ...
            // 셔틀버스 관련 키워드 체크 (스쿨버스비/셔틀버스비 등 모든 변형)
            const isShuttleItem = searchText.includes('셔틀') || searchText.includes('스쿨버스') ||
                                  searchText.includes('통학버스');

            if (!isShuttleItem) return;

            // 월 필터: 10일까지는 전월+당월, 11일부터는 당월만
            if (needPrevMonth) {
                // 10일까지: 전월 또는 당월이면 포함
            ...
            shuttleNameSet.add(row.studentName);
            shuttleDetails.push({ name: row.studentName, month: ym, className: row.className });
        });

        const shuttleNames = Array.from(shuttleNameSet).sort();
        logger.info(`[scrapeMakeEduShuttle] Shuttle students: ${shuttleNames.length}`,
            "names:", JSON.stringify(shuttleNames.slice(0, 30)));
        logger.info(`[scrapeMakeEduShuttle] Details:`, JSON.stringify(shuttleDetails.slice(0, 30)));

        // 5. Firestore shuttle_students 컬렉션 업데이트
        const now = new Date().toISOString();
        ...
        if (existingSnap.size > 0) {
            ...
            for (let i = 0; i < existingSnap.docs.length; i += batchSize) {
                ...
                existingSnap.docs.slice(i, i + batchSize).forEach(doc => deleteBatch.delete(doc.ref));
                await deleteBatch.commit();
            }
        }

        // 셔틀 학생 저장
        if (shuttleNames.length > 0) {
            const batchSize = 450;
            for (let i = 0; i < shuttleNames.length; i += batchSize) {
                const batch = db.batch();
                shuttleNames.slice(i, i + batchSize).forEach(name => {
        ...
        return {
            totalStudents: allBillingRows.length,
            shuttleStudents: shuttleNames.length,
            shuttleNames,
            headers: ['학생명', '청구월', '수업종류', '수업명'],
            monthsChecked: needPrevMonth ? [prevYM, currentYM] : [currentYM],
            syncedAt: now,
        };
...
exports.processConsultationRecording = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        try {
            ...
            const claudeResponse = await fetchClaudeStreamingText("https://api.anthropic.com/v1/messages", {
                method: "POST",
                headers: {
...
화자가 구분되어 있으며, 대화 맥락을 바탕으로 각 화자의 실제 역할(원장, 실장, 선생님, 학부모, 학생 등)이나 이름을 파악해주세요.
보고서 본문에서는 "화자 A", "화자 B" 같은 기호 대신 파악된 역할명(예: [원장], [실장], [학부모], [김OO 선생님])을 직접 사용하세요.
이름이 언급되었으면 "[역할/이름]" 형태로 쓰세요 (예: [원장/홍길동], [학부모/김철수 어머니]).
...
{
  "speakerRoles": {"A": "원장/홍길동", "B": "학부모/김철수 어머니"} (화자 식별 결과. 역할+이름 형태. 이름 파악 불가 시 역할만. 가능한 역할: 원장, 실장, 선생님, 학부모, 학생, 기타),
      ...
      "children": [
        {
          ...
          "children": []
            ...
            if (!claudeResponse.ok) {
                const errBody = claudeResponse.errorBody || "";
                throw new Error(`Claude API 오류: ${claudeResponse.status} - ${errBody}`);
            }

            // streaming helper 가 이미 청크를 누적해 text 필드로 반환
            const reportText = claudeResponse.text || "";
    ...
    .https.onCall(async (data, context) => {
        ...
        try {
            ...
            const [formResponse, analysisResponse] = await Promise.all([
                // (A) 등록상담 폼 필드 추출 (기존)
                fetchClaudeStreamingText("https://api.anthropic.com/v1/messages", {
                    method: "POST",
      ...
      "children": [
        {
          ...
          "children": []
            ...
            // (A) 폼 필드 추출 결과 처리
            if (!formResponse.ok) {
                const errBody = formResponse.errorBody || "";
                throw new Error(`Claude API 오류 (폼 추출): ${formResponse.status} - ${errBody}`);
            }
            // streaming helper 가 이미 청크를 누적해 text 필드로 반환
            const formText = formResponse.text || "";
...
// ============================================================
// regenerateMergedRegistrationReport — 다중 녹음 보고서 AI 통합 재생성 (v2)
// ============================================================
// 사용 시나리오:
//   - 1명 학생, 과목별 따로 녹음 (수학 따로, 영어 따로)
//   - 형제·자매 한 자리 상담, 보고서가 학생별로 N개
//   - 1차 상담 후 일부 누락 → 재녹음 보완
//
// 동작:
//   1. reportIds 배열 검증 (최소 2개)
//   2. 각 보고서 fetch → status==='completed' && transcription 필터
//   3. 시간순 정렬 (createdAt 오름차순)
//   4. 진행 상태 doc 사전 생성 (isMerged=true, status='analyzing')
//   5. 합본 전사 작성 ("=== [세그먼트 K] timestamp ===" 헤더)
//   6. 통합 전용 프롬프트로 Claude 호출 (시간축 통합, 누적 위험 평가 등)
//   7. 응답 파싱 후 보고서 doc 업데이트
//
// 출력: 새 통합 보고서 doc ID. 원본 보고서는 보존.
exports.regenerateMergedRegistrationReport = functions
    .region("asia-northeast3")
    .runWith({
        timeoutSeconds: 540, // 9분 (다중 보고서 Claude 호출은 토큰량 많음)
        memory: "1GB",
        secrets: ["ANTHROPIC_API_KEY"],
    })
    .https.onCall(async (data, context) => {
        ...
        if (!Array.isArray(reportIds) || reportIds.length < 2) {
            throw new functions.https.HttpsError("invalid-argument", "통합하려면 최소 2개의 보고서가 필요합니다.");
        }
        if (reportIds.length > 20) {
            throw new functions.https.HttpsError("invalid-argument", "한 번에 최대 20개까지 통합 가능합니다.");
        }
        const anthropicKey = process.env.ANTHROPIC_API_KEY;
        if (!anthropicKey) {
            throw new functions.https.HttpsError("failed-precondition", "Claude API 키가 설정되지 않았습니다.");
        }

        logger.info(`[regenerateMergedRegistrationReport] Start uid=${context.auth.uid}, reportIds=${reportIds.length}`);

        try {
            // 1. 보고서 fetch (병렬)
            const refs = reportIds.map(id => db.collection("registration_recording_reports").doc(id));
            const snaps = await Promise.all(refs.map(r => r.get()));
            const reports = snaps
                .filter(s => s.exists)
                .map(s => ({ id: s.id, ...s.data() }))
                .filter(r => r.status === "completed" && r.transcription)
                .filter(r => !r.isMerged) // 통합본은 재통합 대상에서 제외
                .sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));

            if (reports.length < 2) {
                throw new functions.https.HttpsError(
                    "failed-precondition",
                    `유효한(완료된) 보고서가 ${reports.length}개로 부족합니다. 최소 2개 필요.`
                );
            }

            // 2. 메타 정보 추출 (첫 보고서 기준 — 같은 상담일/학생/상담자라고 가정)
            const studentName = reports[0].studentName || "미지정";
            const consultationDate = reports[0].consultationDate || "";
            const counselorName = reports[0].counselorName || "";

            // 3. 진행 상태 doc 사전 생성 (사용자가 즉시 onSnapshot으로 진행 추적 가능)
            const mergedRef = db.collection("registration_recording_reports").doc();
            const now = Date.now();
            await mergedRef.set({
                studentName,
                consultationDate,
                counselorName,
                fileName: `통합 분석 (${reports.length}개 보고서)`,
                storagePath: "", // 통합본은 별도 녹음 파일 없음
                fileSizeBytes: 0,
                status: "analyzing",
                statusMessage: `${reports.length}개 보고서 통합 분석 중...`,
                isMerged: true,
                mergedFrom: reports.map(r => r.id),
                mergedAt: now,
                mergedBy: context.auth.uid,
                createdAt: now,
                updatedAt: now,
                createdBy: context.auth.uid,
            });

            // 4. 합본 전사 작성 — 세그먼트 헤더로 구분
            ...
            const combinedTranscript = reports
                .map((r, idx) => {
                    const tsStr = formatTimestamp(r.createdAt);
                    const durStr = r.durationSeconds
                        ? `${Math.floor(r.durationSeconds / 60)}분 ${r.durationSeconds % 60}초`
                        : "";
                    const fileStr = r.fileName || "녹음";
                    const header = `=== [세그먼트 ${idx + 1}] ${tsStr} | ${fileStr}${durStr ? ` (${durStr})` : ""} ===`;
                    return `${header}\n${r.transcription}`;
                })
                .join("\n\n");

            // 5. 첫 보고서의 extractedData에서 studentContext 재구성 (있으면)
            const firstExtracted = reports[0].extractedData || {};
            const studentContext = {
                schoolName: firstExtracted.schoolName,
                grade: firstExtracted.grade,
                parentName: firstExtracted.parentName,
                parentRelation: firstExtracted.parentRelation,
                parentPhone: firstExtracted.parentPhone,
                address: firstExtracted.address,
                birthDate: firstExtracted.birthDate,
                siblings: firstExtracted.siblings,
            };

            // 6. 통합 전용 프롬프트로 Claude 호출
            const claudeHeaders = {
                "x-api-key": anthropicKey,
                "anthropic-version": "2023-06-01",
                "Content-Type": "application/json",
            };
            const mergedPromptBody = `당신은 동일 학생/상담일의 여러 녹음 세그먼트(과목별 또는 추가 녹음)를 종합 분석하는 교육 상담 전문가입니다.

다음은 **시간순으로 정렬된 ${reports.length}개 상담 세그먼트**입니다. 각 세그먼트는 별도 시점/주제로 녹음되었습니다.
각 세그먼트는 \`=== [세그먼트 K] timestamp | filename ===\` 헤더로 구분됩니다.

학생: ${studentName}
상담일: ${consultationDate || "미지정"}
상담자: ${counselorName || "미지정"}
세그먼트 수: ${reports.length}개
${formatStudentContext(studentContext)}
--- 통합 전사본 ---
${combinedTranscript}
--- 끝 ---
...
통합 분석 시 반드시 따를 규칙:
1. **시간축 통합**: 세그먼트 간 시간 순서를 따라 학부모의 의도/감정/관심사가 어떻게 변화했는지 추적하세요.
   예: 세그먼트1에서는 영어만 문의 → 세그먼트2에서 수학도 추가 문의 → 학부모 관심 확장.
2. **누적 위험 평가**: 어느 한 세그먼트에서만 드러난 가벼운 신호도, 다른 세그먼트에서 반복되면 강화된 신호로 분류하세요 (riskFlags).
3. **상담 시퀀스 명시**: 새 필드 \`consultationSequence\`에 각 세그먼트가 무엇이었는지 1줄씩 요약하세요 (예: "1. 수학 진단 결과 설명 (15분)\\n2. 영어 레벨테스트 결과 + 등록 상담 (20분)").
4. **진행 노트**: 새 필드 \`progressNotes\`에 "1차→2차→3차 사이에 학부모의 관심사·감정·결정이 어떻게 변화했는가" 기록하세요.
5. **과목/학생 구분**: 형제·자매 또는 다과목 녹음일 경우 학생별/과목별로 명확히 구분하세요.
6. **합의사항·액션아이템 중복 제거**: 동일 사항이 여러 세그먼트에 나오면 한 번만 기록하되, 가장 구체적인 표현을 채택하세요.
7. ASR 오류 처리: 문맥상 의미가 통하지 않는 부분은 [?]를 붙여주세요.

화자 식별: 각 세그먼트에서 화자가 구분되어 있을 수 있고, 세그먼트마다 다른 화자가 등장할 수 있습니다.
보고서 본문에서는 "화자 A", "화자 B" 같은 기호 대신 파악된 역할(원장/실장/선생님/학부모/학생 등)을 사용하세요.

중요: 각 JSON 값 내에서 항목을 구분할 때 반드시 줄바꿈(\\n)을 사용하세요.
불릿 포인트(-)는 각각 새 줄에 작성하세요.

반드시 아래 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{
  "consultationSequence": "1. (세그먼트1 1줄 요약)\\n2. (세그먼트2 1줄 요약)\\n...",
  "progressNotes": "세그먼트 간 학부모 관심사/감정/결정의 변화 분석 (3-5문장).\\n예: '1차에서는 영어 진단 결과를 듣고 우려 표명 → 2차에서 수학 추가 등록 의사 → 3차에서 셔틀버스 신청까지 확장.'",
  "speakerRoles": {"A": "원장/홍길동", "B": "학부모/김철수 어머니"},
  "consultationType": "통합 상담 성격 요약 (예: '다중 세그먼트 등록 상담 — 수학/영어 진단 + 학부모 우려 해소')",
  "summary": "전체 상담의 통합 요약 (5-7문장). 시간순 흐름 + 핵심 결론 포함.",
  "familyContext": "통합된 가정 배경/개인 맥락. 세그먼트 전체에서 누적된 정보.\\n각 항목은 - 불릿 줄바꿈",
  "parentConcerns": "학부모의 걱정·불안 (세그먼트별 누적, 강화된 신호 우선).\\n각 항목은 - 불릿 줄바꿈",
  "parentQuestions": "학부모 질문사항 (세그먼트별 모두 포함, 답변 여부 구분).\\n형식: - [질문] 내용\\n  → [답변] 또는 [미답변]",
  "parentRequests": "학부모가 직접 발의한 요청 (상담사 제안 제외, 세그먼트별 누적).\\n각 항목은 - 불릿 줄바꿈",
  "parentSatisfaction": "전체 상담에서의 학부모 만족도/감정 추이 분석.",
  "studentNotes": "학생 관련 특이사항 (세그먼트별 누적, 학생별 구분 가능 시 명시).",
  "teacherResponse": "교사/상담사 대응 요약 (세그먼트별 통합).",
  "salesPoints": "상담사가 사용한 설득 논거 (세그먼트별 통합).",
  "agreements": "전체 상담에서 합의된 사항 (중복 제거, 구체적 로드맵 포함).",
  "actionItems": "후속 조치 항목 (전체 통합, 중복 제거).",
  "riskFlags": "주의 신호 (반복되거나 강화된 신호 우선). 절대 '특이사항 없음'으로 넘기지 마세요.",
  "conversationFlow": [
    {
      "topic": "세그먼트 또는 주제명",
      "summary": "1줄 요약",
      "children": [{"topic": "하위", "summary": "1줄", "children": []}]
    }
  ]
}`;
            ...
            const claudeResponse = await fetchClaudeStreamingText("https://api.anthropic.com/v1/messages", {
                ...
                body: JSON.stringify({
                    model: "claude-sonnet-4-6",
                    max_tokens: 64000,
                    messages: [{ role: "user", content: mergedPromptBody }],
                }),
            });

            if (!claudeResponse.ok) {
                const errBody = claudeResponse.errorBody || "";
                throw new Error(`Claude API 오류 (통합 분석): ${claudeResponse.status} - ${errBody}`);
            }

            const responseText = claudeResponse.text || "";

            // 7. 응답 파싱
            let mergedReport;
            try {
                const jsonMatch = responseText.match(/\{[\s\S]*\}/);
                mergedReport = JSON.parse(jsonMatch ? jsonMatch[0] : responseText);
            } catch {
                mergedReport = {
                    summary: responseText,
                    consultationType: "통합 분석 (파싱 실패)",
                    parentConcerns: "",
                    parentQuestions: "",
                    parentRequests: "",
                    studentNotes: "",
            ...
            const speakerRoles = mergedReport.speakerRoles || {};
            delete mergedReport.speakerRoles;

            // 8. 보고서 doc 업데이트 — 통합 결과 저장
            await mergedRef.update({
                status: "completed",
                statusMessage: "통합 분석 완료",
                report: mergedReport,
                speakerRoles,
                transcription: combinedTranscript, // 통합본도 다운로드 가능하도록
                updatedAt: Date.now(),
            });

            logger.info(`[regenerateMergedRegistrationReport] Complete reportId=${mergedRef.id}, sources=${reports.length}`);
            return { reportId: mergedRef.id, status: "completed" };
        } catch (error) {
            logger.error("[regenerateMergedRegistrationReport] Error:", error);
            throw new functions.https.HttpsError(
                "internal",
                error.message || "통합 분석 실패"
            );
        }
    });
...
같은 사람을 가리키는 유사한 이름이 여러 형태로 나타나면 하나로 통일하되, 통일한 사실을 메모해주세요.
화자가 구분되어 있으며, 대화 맥락을 바탕으로 각 화자의 실제 역할과 이름을 파악해주세요.
보고서 본문에서는 "화자 A", "화자 B" 같은 기호 대신 파악된 역할명과 이름(예: [원장/홍길동], [실장/김영희], [수학팀장/박선생])을 직접 사용하세요.
참석자 목록이 제공된 경우 화자와 매칭하여 정확한 이름을 사용하세요.
...
반드시 아래 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{
  "speakerRoles": {"A": "원장/홍길동", "B": "실장/김영희", "C": "수학팀장/박선생"} (역할+이름 형태. 이름 파악 불가 시 역할만),
...
보고서 본문에서는 "화자 A", "화자 B" 같은 기호 대신 파악된 역할명과 이름(예: [원장/홍길동], [실장/김영희])을 직접 사용하세요.
참석자 목록이 제공된 경우 화자와 매칭하여 정확한 이름을 사용하세요.
...
{
  "speakerRoles": {"A": "원장/홍길동", "B": "실장/김영희"} (역할+이름 형태),
  "meetingType": "회의 유형 한 줄 요약",
  "summary": "회의 요약 (5-7문장)",
  "agendaDiscussion": "안건별 논의 내용",
  "decisions": "결정 사항",
...
// ============ 지출결의서 푸시 알림 (v2 — named database 지원) ============
const { onDocumentCreated, onDocumentUpdated } = require("firebase-functions/v2/firestore");

/**
 * 지출결의서 생성 시 결재 대상자(원장/대표/집행자)에게 푸시 알림 발송
 */
exports.notifyExpenseCreated = onDocumentCreated(
    {
    ...
    async (event) => {
        ...
        try {
            const staffSnap = await db.collection("staff")
                .where("expenseRole", "in", ["director", "ceo", "executor"])
                .get();

            if (staffSnap.empty) {
                logger.info("[notifyExpenseCreated] 결재 대상자 없음");
                return null;
            }
            ...
            staffSnap.docs.forEach((doc) => {
                ...
                if (staff.fcmTokens && Array.isArray(staff.fcmTokens)) {
                    tokens.push(...staff.fcmTokens.map((t) => ({
                        token: t,
                        staffId: doc.id,
                    })));
                }
            ...
            if (tokens.length === 0) {
                logger.info("[notifyExpenseCreated] FCM 토큰이 등록된 결재자 없음");
                return null;
            }
            ...
            const message = {
                notification: {
                    title: "지출결의서 결재 요청",
                    body: (expense.author || "") + "님이 '" + (expense.title || "(제목 없음)") + "' 결의서를 제출했습니다. " + amount,
                },
                data: {
            ...
            if (invalidTokens.length > 0) {
                const batch = db.batch();
                const staffTokenMap = {};
                invalidTokens.forEach(({ staffId, token }) => {
                    if (!staffTokenMap[staffId]) staffTokenMap[staffId] = [];
                    staffTokenMap[staffId].push(token);
                });
                ...
                await batch.commit();
                logger.info("[notifyExpenseCreated] 무효 토큰 " + invalidTokens.length + "개 정리");
            }

            const successCount = results.filter((r) => r.status === "fulfilled").length;
            logger.info("[notifyExpenseCreated] 발송 완료: " + successCount + "/" + tokens.length + "건 성공");
            return null;
        } catch (error) {
            logger.error("[notifyExpenseCreated] 오류:", error);
            return null;
        }
    }
);
...
exports.notifyExpenseApproval = onDocumentUpdated(
    ...
    async (event) => {
        ...
        try {
            const roleLabels = { director: "원장", ceo: "대표", executor: "집행자" };
            const roles = ["director", "ceo", "executor"];

            let newlyApprovedRole = null;
            for (const role of roles) {
                const wasBefore = before.approvalChecks?.[role]?.checked || false;
                const isAfter = after.approvalChecks?.[role]?.checked || false;
                if (!wasBefore && isAfter) {
                    newlyApprovedRole = role;
                    break;
                }
            }
...
async function scrapeMakeEduBillingsInternal(fromYm, toYm) {
    ...
    try {
        ...
        // 3-1. studStatus="" (전체학생) — 기본값은 "1"(재원생만)이라 퇴원/휴원생 누락 방지
        // 옵션: ""=전체, "1"=재원, "2"·"3"=휴원/대기, "4"=퇴학
        try {
            await page.waitForSelector('#studStatus', { timeout: 5000 });
            await page.select('#studStatus', '');
            logger.info('[scrapeMakeEduBillings] studStatus set to "" (전체학생)');
        } catch (e) {
            logger.warn('[scrapeMakeEduBillings] studStatus select failed:', e.message);
        }
