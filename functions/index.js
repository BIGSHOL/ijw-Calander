...
- 문맥상 의미가 통하지 않거나 맞지 않는 표현은 [?의심되는 표현] 형식으로 감싸주세요. 예: '[?티켓 찍으려 한다]며 추가적인 스트레스'
- 고유명사(인명, 학교명, 지명)가 잘못 인식된 것 같으면 [?로 감싸주세요. 예: '[?대구일중]'
- 확실히 오인식된 부분은 보고서에 그대로 포함하되, 반드시 [?...] 형식으로 표시하여 사용자가 원본 녹음과 대조할 수 있게 하세요.
...
exports.processRegistrationRecording = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        const { storagePath, studentName, consultationDate, counselorName, fileName } = data;
